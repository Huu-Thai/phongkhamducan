...
<?php
$sql_dmc = "select * from loaitin where idLoaiCha=0";
$danhmucha = mysql_query($sql_dmc);
while($row_dmc = mysql_fetch_array($danhmucha))
{
?>
<option <?php if($row_dmc["idLoaiCha"]==$idCha) echo "selected"; ?> value="<?php echo $row_dmc["idLoaiTin"] ?>"><?php echo $row_dmc["TieuDe"] ?></option>
<?php
}
?>
</select> <a href="main.php?p=dm_xem" class="reloadk">[ Reload Lại Trang ]</a>
<style type="text/css">
	.reloadk:hover
	{
		color: red;
...
<tr>  	<td>{id}</td>
        ...
        <td class="anhien"><a class="smallButton anhien" id="ma_{id}"  AnHien="bang=loaitin&ma=idLoaiTin&id={id}"  href="#"><img  src="images/ah_{AnHien}.png"></a></td>
        <td>
...
<?php
$str = ob_get_clean();
$str = str_replace("{id}",$row_loaisp['idLoaiTin'],$str);
