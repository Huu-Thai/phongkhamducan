...
?>
<?php
	$loaisp = $qt->ListMenu($totalRows, $pageNum,$pageSize, $TieuDe,$idCha);
?>
<style>
...
<script type="text/javascript">
	...
	function suathutu(e,ma)
	{
	    ...
	    typingTimeout = setTimeout( function()
	    {
	    	...
	    	var bien = "id="+ma+"&ttu="+ttu+"&bang=menu";
...
<select onchange="window.location='main.php?p=menu_xem&idCha='+this.value">
<option value="-1">Xem Danh Sách Menu Con</option>
<?php
$sql_dmc = "select * from menu where idCapCha=0";
$danhmucha = mysql_query($sql_dmc);
while($row_dmc = mysql_fetch_array($danhmucha))
{
?>
<option <?php if($row_dmc["idCapCha"]==$idCha) echo "selected"; ?> value="<?php echo $row_dmc["idMenu"] ?>"><?php echo $row_dmc["TieuDe"] ?></option>
<?php
}
?>
</select> <a href="main.php?p=menu_xem" class="reloadk">[ Reload Lại Trang ]</a>
<style type="text/css">
	.reloadk:hover
	{
		color: red;
...
<form action="" method="get" name="form2" id="form2">
	<input type="hidden" name="p" id="p" value="menu_xem" />
...
<tr>  	<td>{id}</td>
        ...
        <td class="anhien"><a class="smallButton anhien" id="ma_{id}"  AnHien="bang=menu&ma=idMenu&id={id}"  href="#"><img  src="images/ah_{AnHien}.png"></a></td>
        <td>
        	<input type="text" name="ThuTu" bien="{id}" id="ThuTu_{id}" class="ThuTu" value="{ThuTu}">
        </td>

      	<td width="100" align="center">
            <a class="smallButton" href="main.php?p=menu_chinh&id={id}"><img  src="images/pencil.png" title="Sửa Danh Mục"></a>
            <a class="smallButton" href="menu_xoa.php?id={id}"  onclick="return confirm('Bạn có muốn xóa !!!');"><img src="images/close.png" title="Xóa Danh Mục"></a>
  		</td>
</tr>
...
<?php
$str = ob_get_clean();
$str = str_replace("{id}",$row_loaisp['idMenu'],$str);
...
$Parent = $row_loaisp['idCapCha'];
if ($Parent > 0)
	$TieuDe = $qt->LayTenMenu($Parent);
else
	$TieuDe = "Không";
